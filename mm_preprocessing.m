...
eye_cascade = vision.CascadeObjectDetector(eyeXMLFile); 
eye_boxes = step(eye_cascade, uint8(output_img));
box_sizes = []; 
sharpening_const = 3.0; 
for i = 1:size(eye_boxes,1)
    box = eye_boxes(i,:); 
...
mouth = imsharpen(mouth, 'Amount', sharpening_const); 
